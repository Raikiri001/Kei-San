...
function Swatch({ color, onPick }: { color: string; onPick: (hex: string) => void }) {
  return (
    <button
      ...
      className="press-scale h-6 w-6 shrink-0 rounded-md border border-[rgb(var(--chrome-border)/0.25)] transition-[border-color,box-shadow] duration-150 hover:border-accent/60 hover:shadow-[0_0_10px_rgb(var(--color-accent-glow)/0.4)]"
  ...
  return (
    <span className="group relative shrink-0">
      ...
      <button
        ...
        className="press-scale absolute -right-1 -top-1 flex h-3 w-3 items-center justify-center rounded-full bg-[rgb(var(--chrome-border))] text-[7px] leading-none opacity-0 transition-opacity duration-150 hover:bg-[rgb(var(--status-inactive-rgb))] group-hover:opacity-100"
      >
        ×
      </button>
    </span>
    ...
    <div className="flex items-center justify-between gap-3">
      ...
      <div className="flex flex-1 flex-wrap items-center gap-1.5">
        ...
        <button
          ...
          className="press-scale flex h-6 w-6 shrink-0 items-center justify-center rounded-md border border-dashed border-[rgb(var(--chrome-border)/0.4)] opacity-70 hover:border-accent/60 hover:opacity-100"
        >
